...
test('Linux systemd definition escapes units and uses only the user directory', () => {
  ...
  assert.match(result.content, /%%a\$\$money\\"quote/); assert.match(result.content, /Restart=no/); assert.doesNotMatch(result.content, /User=root|Environment=.*TOKEN|sudo/);
});
test('macOS uses a per-user LaunchAgent with escaped argument elements', () => {
  ...
  assert.match(result.path, /Library\/LaunchAgents/); assert.match(result.content, /a&amp;b&lt;quote&gt;/); assert.match(result.content, /<key>KeepAlive<\/key><false\/>/);
});
