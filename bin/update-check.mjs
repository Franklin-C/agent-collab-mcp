...
export async function checkUpdate({ force = false, fetcher = fetch, log = console.error } = {}) {
  ...
  try {
    ...
    if (response.status === 404) return { status: 'unreleased', current: manifest.version };
    ...
    mkdirSync(directory, { recursive: true, mode: 0o700 }); writeFileSync(cache, JSON.stringify({ source, repository, name: manifest.name, at: Date.now() }), { mode: 0o600 });
