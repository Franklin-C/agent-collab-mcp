...
import { assertEnrollmentBinding, inspectClient } from './client-adapters.mjs';
import { resetStartupRecovery } from './startup-state.mjs';
...
export function serviceDefinition(input) {
  ...
  if (platform === 'darwin') {
    ...
    return { platform, path: posix.join(home, 'Library/LaunchAgents', `${label}.plist`), scope: 'Current macOS login session.', content: `<?xml version="1.0" encoding="UTF-8"?>\n<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n<plist version="1.0"><dict><key>Label</key><string>${xml(label)}</string><key>ProgramArguments</key><array><string>${xml(node)}</string><string>${xml(launcher)}</string></array><key>RunAtLoad</key><true/><key>KeepAlive</key><dict><key>SuccessfulExit</key><false/></dict><key>ThrottleInterval</key><integer>15</integer><key>ProcessType</key><string>Background</string></dict></plist>\n` };
  }
  if (platform === 'linux') {
    if (!Number.isInteger(uid) || uid <= 0) throw new Error('A logged-in non-root user is required.');
...
export function startupLauncher({ platform, label, state, worker, options, path, codexHome }) {
  ...
  const environment = codexHome ? `process.env.CODEX_HOME = ${JSON.stringify(codexHome)};\n` : 'delete process.env.CODEX_HOME;\n';
  return environment + `import { execFileSync } from 'node:child_process';\nimport { writeFileSync } from 'node:fs';\n${path ? `process.env.PATH = ${JSON.stringify(path)};\n` : ''}const capture = { encoding: 'utf8', windowsHide: true, timeout: 30000, stdio: ['pipe','pipe','pipe'] };\nconst status = (state, reason) => { try { writeFileSync(${JSON.stringify(join(state, 'startup-status.json'))}, JSON.stringify({ at: new Date().toISOString(), state, reason }), { mode: 0o600 }); } catch { /* Status failure must not cause an OS restart loop. */ } };\nconst shutdown = new AbortController();\nlet lifecycle;\nconst stop = () => { shutdown.abort(); lifecycle?.finish('stopped'); };\nprocess.once('SIGINT', stop); process.once('SIGTERM', stop);\ntry {\n  const { beginStartupAttempt } = await import(${JSON.stringify(new URL('./startup-state.mjs', worker).href)});\n  lifecycle = beginStartupAttempt(${JSON.stringify(state)});\n  if (!lifecycle.allowed) { status(lifecycle.phase, lifecycle.reason); } else {\n    process.env.AGENT_COLLAB_TOKEN = ${credential};\n    if (!/^ac_[a-zA-Z0-9_-]+$/.test(process.env.AGENT_COLLAB_TOKEN)) throw new Error('credential unavailable');\n    const { work } = await import(${JSON.stringify(worker)});\n    const { runWithStartupRecovery } = await import(${JSON.stringify(new URL('./startup-recovery.mjs', worker).href)});\n    await runWithStartupRecovery(() => work({ ...${JSON.stringify(options)}, recoverStaleLock: true, signal: shutdown.signal }), { status, signal: shutdown.signal });\n    lifecycle.finish('stopped');\n  }\n} catch { try { lifecycle?.finish('needs_attention'); } catch { /* Retain ambiguous lifecycle state for inspection. */ } status('needs_attention', 'Startup paused. Inspect credentials, permissions, interrupted client work and crash history before resetting recovery.'); }\nfinally { process.removeListener('SIGINT', stop); process.removeListener('SIGTERM', stop); }\n`;
}
...
export function manageStartup(options) {
  ...
  if (!['install', 'status', 'uninstall', 'reset'].includes(action)) throw new Error('Startup action must be install, status, uninstall, or reset.');
  if (action === 'reset') {
    if (!options.resetRecovery) throw new Error('Pass --reset-recovery explicitly after inspecting the stopped worker.');
    return resetStartupRecovery(state);
  }
  if (action === 'status') {
    ...
    return { label, configured: existsSync(metadata), registered, managerAvailable, workerLockPresent: existsSync(join(state, 'worker.lock')), recovery: existsSync(join(state, 'startup-recovery.json')) ? JSON.parse(readFileSync(join(state, 'startup-recovery.json'), 'utf8')) : null, lastRun: existsSync(join(state, 'startup-status.json')) ? JSON.parse(readFileSync(join(state, 'startup-status.json'), 'utf8')) : null, scope: definition.scope };
  }
  if (action === 'uninstall') {
  ...
  assertEnrollmentBinding(prior.enrollment, capability, options);
  ...
  writeFileSync(launcher, startupLauncher({ platform, label, state, worker, options: workerOptions, path: process.env.PATH, codexHome: process.env.CODEX_HOME }), { mode: 0o600, flag: 'wx' });
