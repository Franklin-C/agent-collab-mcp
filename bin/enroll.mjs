...
import { capabilityContract, inspectClient, runClient } from './client-adapters.mjs';
...
export async function enroll(options) {
  ...
  try {
    ...
    await (options.runClient ?? runClient)(capability, prompt, { cwd, env: { ...(options.env ?? process.env), AGENT_COLLAB_TOKEN: token }, model: options.model, write: true, timeoutMs: 180000, signal: options.signal, onActivity: event => activity.record(event, { runId }), onUsage: raw => {
    ...
    state.enrollment = { verifiedAt: new Date().toISOString(), client: capability.client, version: capability.version, repository, capabilities: capabilityContract(capability) }; persist();
