...
export function usageReports(client, raw, model) {
  ...
  if (client === 'codex' && ['turn.completed', 'ehgi.codex_usage_recovery'].includes(event.type) && event.usage && valid(event.usage.input_tokens) && valid(event.usage.output_tokens) && model) return [{model,input_tokens:count(event.usage.input_tokens),output_tokens:count(event.usage.output_tokens),cache_read_tokens:count(event.usage.cached_input_tokens),token_semantics:'inclusive',...(event.type === 'ehgi.codex_usage_recovery' ? {note:'Recovered provider usage from this exact Codex session checkpoint after interruption.'} : {})}];
