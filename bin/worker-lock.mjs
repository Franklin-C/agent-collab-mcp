...
export function acquireWorkerLock(directory, identity, options = {}) {
  ...
  const nonce = randomUUID(), value = JSON.stringify({ version: 1, pid: process.pid, nonce, identity, acquiredAt: new Date().toISOString() });
  ...
  try {
    try { writeFileSync(file, value, { flag: 'wx', mode: 0o600 }); }
      ...
      if (prior?.version !== 1 || prior.identity !== identity || typeof prior.nonce !== 'string' || !/^[a-f0-9-]{36}$/.test(prior.nonce) || !processIsAbsent(prior.pid, options.probe)) throw locked('Worker lock owner is live, inaccessible, from another identity, or unverifiable. It was not removed.');
